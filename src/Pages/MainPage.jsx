...
const MainPage = () => {
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-gray-950 overflow-hidden">
      ...
      <div className="flex justify-between items-center px-4 sm:px-6 py-3 bg-gray-900 bg-opacity-90 z-10">
        <Header title="Overview of Compliance" />
      </div>

      {/* Main Content - flexible height */}
      <div className="flex-1 p-2 sm:p-4 overflow-hidden">
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur-md shadow-lg border border-gray-700 p-3 sm:p-4 md:p-6 h-full rounded-lg">
          <div className="bg-gray-900 p-3 sm:p-4 md:p-6 rounded-xl shadow-xl h-full flex flex-col">
            {/* Section Headers */}
            <div className="flex justify-between mb-2 sm:mb-4">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-100">Material</h2>
              <h2 className="text-xl sm:text-2xl font-bold text-gray-100">Turbine</h2>
            </div>

            {/* Charts Container */}
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
              {/* Material Charts */}
              <div className="w-full md:w-1/2 md:pr-2 lg:pr-4 md:border-r border-gray-700 mb-4 md:mb-0">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 md:gap-4 h-full">
                  {[
                    ...
                    <div
                      key={idx}
                      className="aspect-square sm:aspect-auto flex items-center justify-center h-[calc(33vh-3rem)] sm:h-[calc(33vh-4rem)]"
                    >
                      <PieChart text={chart.text} chartData={chart.data} />
                    </div>
                  ))}
                  {/* Optional fourth chart slot */}
                  <div className="hidden sm:flex items-center justify-center h-[calc(33vh-4rem)]"></div>
                </div>
              </div>

              {/* Turbine Charts */}
              <div className="w-full md:w-1/2 md:pl-2 lg:pl-4 md:border-l border-gray-700">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 md:gap-4 h-full">
                  {[

...
const Maps = () => {
  ...
  const handleTurbineClick = (turbine) => {
    if (SelectedTurbine && SelectedTurbine.TurbineName === turbine.TurbineName) {
      setSelectedTurbine(null)
      setLinePath([])
    } else {
      setSelectedTurbine(turbine)
  ...
  useEffect(() => {
    ...
    const turbineCoords = {
      lat: Number.parseFloat(SelectedTurbine.TurbineLatitude),
      lng: Number.parseFloat(SelectedTurbine.TurbineLongitude),
    }
    ...
    const findClosestWarehouse = () => {
      ...
      let minDistance = haversineDistance(
        turbineCoords.lat,
        turbineCoords.lng,
        Number.parseFloat(closest.Plant_Latitude),
        Number.parseFloat(closest.Plant_Longitude),
      )
      ...
      for (let i = 1; i < warehouses.length; i++) {
        ...
        const dist = haversineDistance(
          turbineCoords.lat,
          turbineCoords.lng,
          Number.parseFloat(wh.Plant_Latitude),
          Number.parseFloat(wh.Plant_Longitude),
        )
    ...
    if (closest) {
      const path = [
        turbineCoords,
        {
          lat: Number.parseFloat(closest.Plant_Latitude),
          lng: Number.parseFloat(closest.Plant_Longitude),
        },
      ]
      setLinePath(path)
    } else {

...
const ChartComponent = ({type}) => {
  ...
  const formattedData = TurbineData.map(turbine => ({
    ...
    nominal_power: parseFloat(turbine.NominalPower.replace(' KW', '').replace(',', '.'))
  }));
        ...
        <div className="flex justify-center items-center w-full h-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={formattedData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <YAxis />
              <Tooltip />
              <Bar dataKey="nominal_power" fill="#8884d8" />
              <Legend />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="flex justify-center items-center w-full h-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={formattedData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="nominal_power" stroke="#8884d8" />
              <Legend />
            </LineChart>
          </ResponsiveContainer>
        </div>
